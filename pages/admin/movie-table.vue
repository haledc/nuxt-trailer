<template>
  <div class="movie-table">
    <div class="title-container">
      ...
      <el-button class="logout" size="mini" type="info" @click="logout">
        退出
      </el-button>
    </div>
    <div class="table-container">
      <el-table :data="filterMovieList" border style="width: 100%">
        <el-table-column prop="poster" label="海报" align="center" width="180">
          <template slot-scope="scope">
            <img :src="scope.row.poster" alt="poster" width="60" height="90" />
          </template>
        </el-table-column>
        <el-table-column prop="title" label="名称" align="center" width="200">
        </el-table-column>
        <el-table-column
          prop="pubdate"
          label="上映时间"
        ...
        <el-table-column prop="tags" label="标签" align="center" width="350">
          <template slot-scope="scope">
            <el-tag
        ...
        <el-table-column prop="detail" label="详情" align="center" width="180">
          <template slot-scope="scope">
            <el-button
              type="primary"
              class="button"
              @click="goDetail(scope.row._id)"
            >
        ...
        <el-table-column prop="delete" label="删除" align="center">
          <template slot-scope="scope" style="color: red;">
            <el-button
              type="danger"
              class="button"
              @click="deleteMovie(scope.row._id)"
            >
...
<script>
import { normalizeAdminMovies } from '../../assets/helper'
...
export default {
  ...
  beforeCreate() {
    this.$axios.get('/api/admin/movie/list').then(res => {
      if (res.data.success) {
  ...
  methods: {
    ...
    setPagination() {
      ...
      this.filterMovieList = this.adminMovieList.filter(
    ...
    handleCurrentChange(pageIndex) {
      ...
      const restTableData = this.adminMovieList.filter(
        (item, index) => index >= startNum
      )
      this.filterMovieList = restTableData.filter(
        (item, index) => index < this.pagination.pageSize
      )
    }
  }
